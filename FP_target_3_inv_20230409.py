...
candle_count = round((60/time_unit) * 24 * 3.5)
...
ema_Not_buy_cri = 1.0
buy_price_up_unit = 1
ma_trend_check_duration = 70   # 구매가 적정한 구간인지, 최근의 전반적인 상승장 수중 확인
ratio_average_duraion_long = 20   # 두 가지 참조 구간중, 더 오래된 참조시작 시점 설정
ratio_average_duraion_short = 5   # 두 가지 참조 구간중, 더 근래의 참조시작 시점 설정
...
sell_normal_profit_ratio = 0.06   # 정상 판매 이익율
...
def DF_mv (DF_mv_target) :
    ...
    DF_mv_target['ema_open_short_mid'] = DF_mv_target['open'].ewm(span = 15, adjust=False).mean()
    
    DF_mv_target['ma_trend_check'] = DF_mv_target['open'].ewm(span=ma_trend_check_duration, adjust=False).mean()
    DF_mv_target['ratio_trend'] = DF_mv_target['ma_trend_check'] / DF_mv_target['ma_trend_check'].shift(1)
    ...
    try :
        ...
        if (now.minute % time_unit == 1) & (0 < (now.second % 60) <= 15) :   # N시:01:00초 ~ N시:01:15초 사이 시각이면
            ...
            if bought_state == 0 :   # 매수가 없는 상태라면
                ...
                print ('\n[BUY condition check]')
                print ('Sell forced previously : {0}'.format(Sell_Forced))
                ...
                print ('DF_test_ma[ema_open_mid][-2] / DF_test_ma[ema_open_mid][-3]) : {0} greater than {1}'.format((DF_test_ma['ema_open_mid'][-2] / DF_test_ma['ema_open_mid'][-3]), ratio_sell_ema_mid_chg_state))
                print ('DF_test_ma[ema_open_mid][-1] / DF_test_ma[ema_open_mid][-2]) : {0} greater than {1}'.format((DF_test_ma['ema_open_mid'][-1] / DF_test_ma['ema_open_mid'][-2]), ratio_sell_ema_mid_chg_state))
                print ('DF_test_ma[ratio_trend][-1] : {0} greater than (0.9998 * DF_test_ma.iloc[-(ratio_average_duraion_long) : -1, :][ratio_trend].mean()) : {1}.'.format(DF_test_ma['ratio_trend'][-1], (0.9998 * DF_test_ma.iloc[-(ratio_average_duraion_long) : -1, :]['ratio_trend'].mean())))
                print ('DF_test_ma[ratio_trend][-1] : {0} greater than (0.9998 * DF_test_ma.iloc[-(ratio_average_duraion_hort) : -1, :][ratio_trend].mean()) : {1}.'.format(DF_test_ma['ratio_trend'][-1], (0.9998 * DF_test_ma.iloc[-(ratio_average_duraion_short) : -1, :]['ratio_trend'].mean())))  
                print ('DF_test_ma[open][-1] / DF_test_ma[open][-3]) : {0} greater than 1.0'.format (DF_test_ma['open'][-1] / DF_test_ma['open'][-3]))                
                
                # 매수 영역
                
                if ((Sell_Forced == 0) and                     (DF_test_ma['ema_o_l_consecutive_rise'][-(ma_duration_long + 1) : -1].sum() > 5) and                     (DF_test_ma['ema_o_s_consecutive_rise'][-(ma_short_under_duration + 1) : -1].sum() > (ma_short_under_duration - 2)) and                     ((DF_test_ma['ema_open_long'][-1] / DF_test_ma['ema_open_long'][-2]) > ratio_ema_long) and                     ((DF_test_ma['ratio_vol_to_aver'][-3] > buy_cri_vol_times) or (DF_test_ma['ratio_vol_to_aver'][-2] > buy_cri_vol_times)) and                     (DF_test_ma['ema_open_short'][-1] / DF_test_ma['ema_open_long'][-1] > ratio_ema_o_short_long_buy_1[0]) and (DF_test_ma['ema_open_short'][-1] / DF_test_ma['ema_open_long'][-1] < ratio_ema_o_short_long_buy_1[1]) and                     (DF_test_ma['ratio_ema_Not_buy_check'][-1] > ema_Not_buy_cri) and                     ((DF_test_ma['ema_open_mid'][-2] / DF_test_ma['ema_open_mid'][-3]) >= ratio_sell_ema_mid_chg_state) and ((DF_test_ma['ema_open_mid'][-1] / DF_test_ma['ema_open_mid'][-2]) >= ratio_sell_ema_mid_chg_state)and                     (DF_test_ma['ratio_trend'][-1] > (0.9998 * DF_test_ma.iloc[-(ratio_average_duraion_long) : -1, :]['ratio_trend'].mean())) and                     (DF_test_ma['ratio_trend'][-1] > (0.9998 * DF_test_ma.iloc[-(ratio_average_duraion_short) : -1, :]['ratio_trend'].mean())) and                     ((DF_test_ma['open'][-1] / DF_test_ma['open'][-3]) >= 1.0)) :
        ...
        if bought_state == 1 :   # 매수가 되어 있는 상태라면
            ...
            print ('current_price is : {0}  / bought_price : {1}'.format(pyupbit.get_current_price(coin_inv), get_avg_buy_price(LIST_check_coin_currency[coin_No])))
            ...
            print ('current_price / bought_price : {0} / less than (1 - sell_force_loss : {1}'.format((pyupbit.get_current_price(coin_inv) / get_avg_buy_price(LIST_check_coin_currency[coin_No])), (1 - sell_force_loss)))
                                                                                                      
            if (pyupbit.get_current_price(coin_inv) / get_avg_buy_price(LIST_check_coin_currency[coin_No]) < (1 - sell_force_loss)) :   # 강제 매도 가격 이하로 현재가격이 하락하게 되면
        ...
        if (now3.minute % time_unit == 1) & (0 < (now.second % 60) <= 15) :   # N시:01:00초 ~ N시:01:15초 사이 시각이면
            ...
            if bought_state == 1 :   # 매수가 되어 있는 상태라면
                ...
                if (((((datetime.datetime.now() + datetime.timedelta(seconds = (time_factor * 3600))) - buy_time).seconds / 3600) < 5.0) and                     ((DF_mov_avg_check_SELL['ema_open_mid'][-2] / DF_mov_avg_check_SELL['ema_open_mid'][-3]) < ratio_sell_ema_mid_chg_state) and ((DF_mov_avg_check_SELL['ema_open_mid'][-1] / DF_mov_avg_check_SELL['ema_open_mid'][-2]) < ratio_sell_ema_mid_chg_state)) or                 (((((datetime.datetime.now() + datetime.timedelta(seconds = (time_factor * 3600))) - buy_time).seconds / 3600) >= 5.0) and                     ((DF_mov_avg_check_SELL['ema_open_short_mid'][-2] / DF_mov_avg_check_SELL['ema_open_short_mid'][-3]) < ratio_sell_ema_mid_chg_state) and ((DF_mov_avg_check_SELL['ema_open_short_mid'][-1] / DF_mov_avg_check_SELL['ema_open_short_mid'][-2]) < ratio_sell_ema_mid_chg_state)) :
